...
namespace nnvk::layout {
    ...
    size_t GetBlockLinearLayerSize(vk::Extent3D dimensions, size_t formatBlockWidth, size_t formatBlockHeight, size_t formatBpb, size_t gobBlockHeight, size_t gobBlockDepth) {
        ...
        size_t robDepth{util::AlignUp<size_t>(dimensions.depth, gobBlockDepth)}; //!< The depth of the surface in slices, aligned to include padding Z-axis GOBs
    ...
    size_t GetBlockLinearLayerSize(vk::Extent3D dimensions, size_t formatBlockHeight, size_t formatBlockWidth, size_t formatBpb, size_t gobBlockHeight, size_t gobBlockDepth, size_t levelCount, bool isMultiLayer) {
    ...
    std::vector<MipLevelLayout> GetBlockLinearMipLayout(vk::Extent3D dimensions, size_t formatBlockHeight, size_t formatBlockWidth, size_t formatBpb, size_t targetFormatBlockHeight, size_t targetFormatBlockWidth, size_t targetFormatBpb, size_t gobBlockHeight, size_t gobBlockDepth, size_t levelCount) {
        ...
        for (size_t i{}; i < levelCount; i++) {
            ...
            mipLevels.emplace_back(
                ...
                linearSize,
                targetLinearSize,
                (GobWidth * gobsWidth) * (GobHeight * util::AlignUp(gobsHeight, gobBlockHeight)) * util::AlignUp<size_t>(dimensions.depth, gobBlockDepth),
    ...
    void CopyBlockLinearInternal(vk::Extent3D dimensions,
        ...
        size_t blockDepth{util::AlignUp<size_t>(dimensions.depth, gobBlockDepth) - dimensions.depth};
        ...
        auto deswizzleRob{[&](u8 *pitchRob, auto isLastRob, size_t depthSliceCount, size_t blockPaddingY = 0, size_t blockExtentY = 0) {
            auto deswizzleBlock{[&](u8 *pitchBlock, auto copySector) __attribute__((always_inline)) {
                ...
                if (depthSliceCount != gobBlockDepth)
                    [[unlikely]]
     ...
     */
    template<bool BlockLinearToPitch>
    void CopyBlockLinearSubrectInternal(vk::Extent3D pitchDimensions, vk::Extent3D blockLinearDimensions,
                                        size_t formatBlockWidth, size_t formatBlockHeight, size_t formatBpb, u32 pitchAmount,
                                        size_t gobBlockHeight, size_t gobBlockDepth,
                                        u8 *blockLinear, u8 *pitch,
        ...
        size_t lastMobSliceCount{gobBlockDepth - (util::AlignUp<size_t>(blockLinearDimensions.depth, gobBlockDepth) - blockLinearDimensions.depth)};
            ...
            for (size_t currMob{}; currMob < depthMobCount; ++currMob, blockLinear += robSize * robPerMob) {
                ...
                for (size_t slice{}; slice < sliceCount; ++slice, sliceOffset += (GobHeight * GobWidth * gobBlockHeight)) {
                    ...
                    for (size_t line{}; line < pitchTextureHeight; ++line, pitchOffset += pitchBytes) {
                        ...
                        if (line && !((originY + line) & (robHeight - 1)))
                            [[unlikely]]
                                robOffset += robSize;
    ...
    void CopyBlockLinearToLinear(vk::Extent3D dimensions, size_t formatBlockWidth, size_t formatBlockHeight, size_t formatBpb, size_t gobBlockHeight, size_t gobBlockDepth, u8 *blockLinear, u8 *linear) {
    ...
    void CopyPitchToBlockLinear(vk::Extent3D dimensions, size_t formatBlockWidth, size_t formatBlockHeight, size_t formatBpb, u32 pitchAmount, size_t gobBlockHeight, size_t gobBlockDepth, u8 *pitch, u8 *blockLinear) {
    ...
    static constexpr u8 BlockLinearTileSizeFromGobs(size_t gobs) {
        if (gobs >= 16)
            return 16;
        else if (gobs >= 8)
            return 8;
    ...
    u8 SelectBlockLinearTileHeight(size_t height, size_t formatBlockHeight) {
        size_t heightBlocks{util::DivideCeil(height, formatBlockHeight)};
        size_t heightAndHalf{heightBlocks + (heightBlocks / 2)};

        size_t heightAndHalfGobs{util::DivideCeil<size_t>(heightAndHalf, GobHeight)};
        return BlockLinearTileSizeFromGobs(heightAndHalfGobs);
    }

    u8 SelectBlockLinearTileDepth(size_t depth) {
        size_t depthAndHalf{depth + (depth / 2)};
        return BlockLinearTileSizeFromGobs(depthAndHalf);
    }
}
